...
class Main {
	...
	static function readYYDebug(input:YyInput) {
		...
		for (i in 0 ... positions.length) {
			...
			} else if (name.startsWith("gml_GlobalScript_")) {
				// GMS 2.3+ scripts are all 'global scripts' too
				// but if it's just a number then it's probably an autogenerated global script
				// (e.g. gml_pragma)
				name = name.substring(17);
				// script name starts with a number? must be autogenerated then.
				if (Std.parseInt(name) != null) name = "GlobalScript:" + name;
			} else if (name.startsWith("gml_Trigger_")) {
				// technically unused but you can still compile it manually.
				// ah, good old gm8 times.
				name = name.substring(12);
			} else if (name.startsWith("gml_GameEndScript_")) {
				// always autogenerated. gml_pragma("AtGameEnd", @"show_error(222, true);"); ->
				// gml_GlobalScript_2
				name = "GameEndScript:" + name.substring(18);
			} else if (name.startsWith("Timeline_")) {
				// Timeline_timelineAssetName_momentIndex
				name = name.substring(9);
				p1 = name.lastIndexOf("_"); // index of before moment underscore
				s1 = name.substring(p1 + 1); // moment index string
				name = name.substring(0, p1); // timeline name
				name = name + ":" + s1; // tml_test:2
